...
const placeOrder = async (req, res) => {
    try {
        ...
        console.log(error);
        res.json({ success: false, message: MESSAGES.orderError });
    }
};
...
const placeOrderCod = async (req, res) => {
    try {
        ...
        res.json({ success: true, message: MESSAGES.orderPlaced });
...
const updateStatus = async (req, res) => {
    try {
        await orderModel.findByIdAndUpdate(req.body.orderId, { status: req.body.status, deliveryPartner: req.body.deliveryPartnerId });
        res.json({ success: true, message: MESSAGES.statusUpdated });
    } catch (error) {
        res.json({ success: false, message: MESSAGES.orderError });
    }
};
...
const verifyOrder = async (req, res) => {
    ...
    try {
        if (success === "true") {
            ...
            await orderModel.findByIdAndDelete(orderId);
            res.json({ success: false, message: MESSAGES.notPaid });
        }
    } catch (error) {
        res.json({ success: false, message: MESSAGES.notVerified });
    }
};
